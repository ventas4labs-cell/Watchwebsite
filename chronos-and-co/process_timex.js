...
const RAW_DATA_PATH = path.join(__dirname, 'timex_raw.json');
...
for (const item of rawData) {
    ...
    // Generate ID: timex-{sku}
    let cleanSku = item.sku.replace(/^TXTW/, 'TW'); // shorten if it has prefix
    const id = `timex-${item.sku.toLowerCase().replace(/[^a-z0-9]/g, '')}`;
...
console.log(`Found ${newItems.length} new unique Timex models.`);
...
async function processItems() {
    for (const item of newItems) {
        ...
        // Remove resizing params if present
        ...
        try {
            ...
            console.error(`Failed to download ${item.id} clean, trying params...`);
            try {
        ...
        if (item.price) {
            price = Math.round(parseFloat(item.price.replace(/[^0-9.]/g, '')) * 100);
        }
    ...
    price: ${price},
    description: "Official Timex ${item.name.replace(/"/g, '\\"')} (${item.sku}). Reliable, durable, and iconic.",
    ...
    fs.writeFileSync('timex_snippet.ts', code);
    console.log("Snippet saved to timex_snippet.ts");
}
